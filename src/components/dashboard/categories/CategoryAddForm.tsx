...
export function CategoryAddForm({
  ...
  return (
    <div className="bg-white dark:bg-[#112240] p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-white/10 space-y-6 shadow-sm">
      <h2 className="text-lg sm:text-xl font-bold text-amber-800 dark:text-pharaohGold border-b border-slate-200 dark:border-white/10 pb-3">إضافة تصنيف عمل جديد</h2>
      <form onSubmit={onAdd} className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
        <div>
          <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-2">اسم التصنيف (بالعربية)</label>
          <input
            type="text"
            required
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-2">Name (English)</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-2">معرف التصنيف / Slug (اختياري)</label>
          <input
            type="text"
        ...
        <div className="md:col-span-3 flex justify-end">
          <button
            type="submit"
            disabled={submitting}
            className="bg-pharaohGold text-[#0A192F] px-8 py-3.5 rounded-xl font-black text-sm hover:bg-white transition-all shadow-lg disabled:opacity-50 cursor-pointer"
          >
