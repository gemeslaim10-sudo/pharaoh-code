...
export default function CreativityServicesForm({ onSuccess }: Props) {
    ...
    return (
        <form onSubmit={handleSubmit} className="db-form-content bg-white dark:bg-[#112240] border border-slate-200 dark:border-white/5 rounded-3xl p-6 lg:p-10 shadow-md dark:shadow-2xl relative">
            <div className="absolute top-0 left-10 transform -translate-y-1/2 bg-pharaohGold text-[#0A192F] font-black text-[10px] uppercase tracking-[3px] px-4 py-1.5 rounded-full shadow-lg">ROYAL DIGITAL SERVICES</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    <label className="block text-xs font-bold text-amber-800 dark:text-pharaohGold uppercase tracking-wider mb-2">مسمى الخدمة الرقمية (بالعربية)</label>
                    <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-slate-50 dark:bg-[#0A192F] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3.5 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:border-pharaohGold transition" placeholder="مثال: هندسة النظم السحابية الفرعونية" />
                </div>
                <div>
                    <label className="block text-xs font-bold text-amber-800 dark:text-pharaohGold uppercase tracking-wider mb-2">مسمى الخدمة (بالإنجليزية - Title EN)</label>
                    <input type="text" value={titleEn} onChange={e => setTitleEn(e.target.value)} className="w-full bg-slate-50 dark:bg-[#0A192F] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3.5 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:border-pharaohGold transition" placeholder="e.g. Cloud Systems Engineering" dir="ltr" />
                </div>

                <div>
                    <label className="block text-xs font-bold text-amber-800 dark:text-pharaohGold uppercase tracking-wider mb-2">نص زر الإجراء والاتصال</label>
                    <input type="text" required value={btnText} onChange={e => setBtnText(e.target.value)} className="w-full bg-slate-50 dark:bg-[#0A192F] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3.5 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:border-pharaohGold transition" placeholder="مثال: اعتلاء السحابة الرقمية" />
                </div>
                <div>
                    <label className="block text-xs font-bold text-amber-800 dark:text-pharaohGold uppercase tracking-wider mb-2">كود أيقونة الخدمة (SVG)</label>
                ...
                <div>
                    <label className="block text-xs font-bold text-amber-800 dark:text-pharaohGold uppercase tracking-wider mb-2">وصف الخدمة (بالعربية)</label>
                ...
                <div>
                    <label className="block text-xs font-bold text-amber-800 dark:text-pharaohGold uppercase tracking-wider mb-2">وصف الخدمة (بالإنجليزية - Description EN)</label>
