...
export function ServiceFormOverviewTab({
  ...
  const handleAddFeature = () => {
    if (addFeature) {
      ...
      setFeatures([...features, { title_ar: '', title_en: '', desc_ar: '', desc_en: '' }]);
      setActiveFeatureIdx(features.length);
    }
  };
  ...
  const handleRemoveFeature = (idx: number) => {
    ...
    if (activeFeatureIdx >= features.length - 1) {
      setActiveFeatureIdx(Math.max(0, features.length - 2));
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Panel 2: Key Features (Unlimited Dynamic Count) */}
      <div className="bg-slate-50 dark:bg-[#0A192F] p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-white/10 space-y-5 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
          <div>
            <h5 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
              <span className="text-amber-800 dark:text-pharaohGold text-lg">⭐</span>
              <span>المميزات الرئيسية للخدمة ({features.length} Key Features)</span>
            </h5>
            <p className="text-xs text-slate-500 dark:text-gray-400 mt-0.5">يمكنك إضافة أي عدد من المميزات لعرضها في صفحة تفاصيل الخدمة.</p>
          </div>
