    ...
    <div className="overflow-x-auto">
      <table className="w-full text-right text-sm text-slate-600 dark:text-gray-400">
        ...
        <tbody>
          ...
          {/* Dynamic Admins */}
          {admins.map((admin) => (
            <tr key={admin.id} className="border-b border-slate-200 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition">
              <td className="px-6 py-4 font-bold text-slate-900 dark:text-white">{admin.email}</td>
              <td className="px-6 py-4 text-center">
                <button
                  onClick={() => onDelete(admin.id)}
                  className="text-xs text-red-600 dark:text-red-400 bg-red-500/10 border border-red-500/20 px-3 py-1 rounded-full hover:bg-red-500 hover:text-white transition cursor-pointer font-bold"
                >
                  إلغاء الصلاحية ❌
                </button>
              </td>
