    ...
    return (
        <div className="bg-[#112240] p-8 rounded-[2.5rem] border border-white/5 hover:border-pharaohGold/20 shadow-2xl transition-all duration-500 mt-10">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-6">
                ...
                <h4 className="text-xl font-bold text-white">أعضاء الفريق الحاليين</h4>
            </div>

            {loading ? (
                <div className="text-center text-pharaohGold py-10 font-bold">جاري تحميل أعضاء الفريق...</div>
            ) : members.length === 0 ? (
                            ...
                            {members.map((member) => (
                                <tr key={member.id} className="border-b border-white/5 hover:bg-white/5 transition">
                                    ...
                                    <td className="px-6 py-4 text-center space-x-2 space-x-reverse">
                                        <button onClick={() => onEdit(member)} className="text-xs text-pharaohGold bg-[#112240] border border-pharaohGold/30 px-3 py-1.5 rounded-full hover:bg-pharaohGold hover:text-[#0A192F] transition cursor-pointer">
                                            تعديل
                                        </button>
                                        <button onClick={() => onDelete(member.id)} className="text-xs text-red-400 bg-red-400/10 px-3 py-1.5 rounded-full hover:bg-red-400 hover:text-white transition cursor-pointer">
                                            حذف
                                        </button>
                                    </td>
                                </tr>
