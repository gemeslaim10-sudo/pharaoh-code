    ...
    <div className="pt-5 border-t border-white/10 flex flex-col gap-3">
      ...
      >
        <Link
          href="/start-project"
          onClick={onClose}
          className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-[#C5A16F] via-[#D4AF37] to-[#C5A16F] text-[#050B14] font-black text-center text-sm shadow-[0_10px_25px_rgba(197,161,111,0.3)] flex items-center justify-center gap-2"
        >
      ...
      {user ? (
        <div className="flex flex-col gap-2">
          ...
          <div className="p-2.5 rounded-xl bg-white/5 border border-white/10 flex items-center justify-between gap-2 shadow-sm">
          ...
          {isAdmin && (
            <motion.div whileTap={{ scale: 0.96 }}>
              <Link
                href="/dashboard"
                onClick={onClose}
                className="w-full py-2.5 rounded-xl bg-[#C5A16F]/10 border border-[#C5A16F]/40 text-[#C5A16F] font-bold text-xs text-center flex items-center justify-center gap-1.5 hover:bg-[#C5A16F] hover:text-[#050B14] transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <motion.div whileTap={{ scale: 0.96 }}>
          <Link
            href="/login"
            onClick={onClose}
            className="w-full py-3 rounded-xl bg-white/5 border border-white/15 text-gray-200 hover:text-white hover:border-[#C5A16F] font-bold text-xs text-center flex items-center justify-center gap-2 transition-colors shadow-sm"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
