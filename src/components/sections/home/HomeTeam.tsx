...
export default function HomeTeam({ data }: { data?: SectionData }) {
  ...
  return (
    <section id="our-team" className="relative py-14 sm:py-20 bg-[#050B14] overflow-hidden text-white select-none" dir={direction}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        {/* Section Header with Monumental Layout & View All Link */}
        ...
        {/* High-End Team Elite Cards Grid */}
          ...
          {membersToRender.slice(0, 4).map((member: SectionItem, index: number) => {
            ...
            return (
              <Link 
                href={`/team/${member.id}`} 
                key={index} 
                className="team-card group relative block rounded-2xl p-4 sm:p-4.5 bg-gradient-to-b from-[#0F1E38] via-[#091528] to-[#050B14] border border-white/5 hover:border-[#C5A16F]/40 transition-all duration-400 hover:-translate-y-1 shadow-[0_12px_35px_rgba(0,0,0,0.6)] overflow-hidden"
              >
                {/* Top Glowing Beam */}
                ...
                <div className="relative aspect-[4/3] sm:aspect-[3/4] rounded-xl overflow-hidden mb-3.5 bg-[#0B1528]">
                  {memberImage ? (
                    <img 
                      src={memberImage} 
                      className="w-full h-full object-cover grayscale contrast-110 group-hover:grayscale-0 group-hover:contrast-100 transition-all duration-500 transform group-hover:scale-105" 
                      alt={memberName} 
                    />
                  ) : (
                  ...
                  {/* Dramatic Bottom Vignette Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-[#050B14] via-[#050B14]/20 to-transparent opacity-80 group-hover:opacity-30 transition-opacity duration-400" />
                </div>

                {/* Member Info */}
                <div className="px-1 pb-1">
                  <h4 className="text-white text-lg font-bold tracking-tight mb-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-[#F3E0B5] transition-all">
                    {memberName}
                  </h4>
                  
                  <p className="text-[#C5A16F] text-xs font-medium uppercase tracking-wider mb-3">
                    {memberRole}
                  </p>

                  {/* Profile Action Line */}
                  <div className="pt-2.5 border-t border-white/5 flex items-center justify-between text-xs text-gray-400 group-hover:text-[#C5A16F] transition-colors">
