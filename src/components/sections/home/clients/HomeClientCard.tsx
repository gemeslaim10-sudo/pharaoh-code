  ...
  return (
    <div 
      className={`group relative rounded-3xl p-5 sm:p-6 border transition-all duration-500 flex flex-col justify-between overflow-hidden h-full hover:-translate-y-2.5 select-none ${
        isLight
          ? 'bg-white border-slate-300 hover:border-[#8A5800] shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:shadow-[0_20px_45px_-8px_rgba(138,88,0,0.2)]'
    ...
    >
      ...
      <div>
        ...
        <Link href={`/clients/${client.id}`} className="block relative z-10">
          <div className={`relative w-full h-36 sm:h-40 rounded-2xl overflow-hidden mb-4 border flex items-center justify-center p-6 transition-all duration-500 shadow-inner group/banner ${
            isLight 
              ? 'bg-white border-slate-300 group-hover:border-[#8A5800]/50 shadow-slate-100 group-hover:shadow-[0_8px_25px_rgba(138,88,0,0.15)]' 
            ...
            <div className={`absolute top-2.5 ${direction === 'rtl' ? 'right-2.5' : 'left-2.5'} px-2.5 py-0.5 rounded-full text-[9px] font-black tracking-wider uppercase border shadow-sm backdrop-blur-md z-20 ${
              isLight 
                ? 'bg-amber-50 text-[#8A5800] border-[#8A5800]/40' 
        ...
        <Link href={`/clients/${client.id}`} className="block mb-2 relative z-10">
          <div className="flex items-center gap-2">
            <span className="text-[#C5A16F] text-xs drop-shadow-[0_0_6px_rgba(197,161,111,0.5)]">✦</span>
        ...
        <p className={`text-xs sm:text-sm leading-relaxed line-clamp-2 sm:line-clamp-3 mb-4 relative z-10 ${
      ...
      <div className={`pt-3.5 border-t flex items-center justify-between gap-2 relative z-10 ${
        isLight ? 'border-slate-200' : 'border-white/10'
      }`}>
        <Link 
          href={`/clients/${client.id}`} 
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl font-black text-[11px] sm:text-xs whitespace-nowrap shrink-0 transition-all duration-300 shadow-sm border ${
            isLight
              ? 'bg-amber-50 border-[#8A5800]/40 text-[#8A5800] hover:bg-[#8A5800] hover:text-white hover:shadow-[0_4px_16px_rgba(138,88,0,0.35)]'
        ...
        >
          <span>{t("clients.projectDetails") || (language === 'ar' ? 'التفاصيل' : 'Details')}</span>
          <svg
        ...
        {client.websiteUrl && (
          <a 
            ...
            className={`inline-flex items-center gap-1 text-[11px] sm:text-xs font-bold whitespace-nowrap shrink-0 transition-all py-1.5 px-2.5 sm:px-3 rounded-lg border ${
              isLight 
                ? 'text-slate-800 hover:text-slate-950 hover:bg-slate-100 border-slate-300' 
                : 'text-gray-300 hover:text-[#C5A16F] hover:bg-white/5 border-white/10 hover:border-[#C5A16F]/30'
            }`}
          >
            <span>{t("clients.visitWebsite") || (language === 'ar' ? 'زيارة' : 'Visit')}</span>
            <svg className="w-3 h-3 opacity-70 group-hover:opacity-100 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
