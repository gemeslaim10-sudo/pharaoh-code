        ...
        <div className="lg:col-span-4 space-y-6">
            <div className="bg-[#112240] p-8 rounded-[2.5rem] border border-white/5 hover:border-[#C5A16F]/30 transition-all group relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <h4 className="text-white text-xl font-bold mb-2">{getDynamicText(data.info || {}, 'emailTitle', language) || (language === 'ar' ? "المراسلة الفورية" : "Instant Email")}</h4>
                    ...
                    <a href={`mailto:${data.info?.email || "info@pharaohcode.com"}`} className="text-[#C5A16F] text-xs font-bold hover:underline">
                        {getDynamicText(data.info || {}, 'emailLinkText', language) || (language === 'ar' ? "أرسل إيميل مباشر ←" : "Send Direct Email →")}
                    </a>
                </div>
            </div>
            ...
            <div className="bg-[#112240] p-8 rounded-[2.5rem] border border-white/5 hover:border-[#C5A16F]/30 transition-all group">
                ...
                <h4 className="text-white text-xl font-bold mb-2">{getDynamicText(data.info || {}, 'addressTitle', language) || (language === 'ar' ? "زيارة المقر" : "Visit Office")}</h4>
                <p className="text-gray-400 text-sm">{getDynamicText(data.info || {}, 'address', language) || (language === 'ar' ? "القاهرة، مصر - التجمع الخامس" : "Cairo, Egypt - 5th Settlement")}</p>
            </div>
            ...
            <div className="bg-[#112240] p-8 rounded-[2.5rem] border border-white/5 hover:border-[#C5A16F]/30 transition-all group">
                ...
                <h4 className="text-white text-xl font-bold mb-2">{getDynamicText(data.info || {}, 'phoneTitle', language) || (language === 'ar' ? "تحدث معنا فوراً" : "Call Us Now")}</h4>
                ...
                <a href={data.info?.whatsappLink || "#"} className="inline-block bg-[#25D366] text-white px-6 py-2 rounded-full text-xs font-bold hover:bg-opacity-80 transition-all">
                    {getDynamicText(data.info || {}, 'whatsappText', language) || (language === 'ar' ? "فتح واتساب" : "Open WhatsApp")}
                </a>
            </div>
        </div>
    );
