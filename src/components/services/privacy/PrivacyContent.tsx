...
export default function PrivacyContent({ data }: PrivacyContentProps) {
    ...
    return (
        <section className="pb-24 relative overflow-hidden text-right" dir="rtl">
            ...
            <div className="absolute inset-x-0 bottom-1/3 flex items-center justify-center pointer-events-none select-none opacity-[0.012] z-0">
            ...
            <div className="max-w-6xl mx-auto px-6 relative z-10" style={{ marginTop: "40px" }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {data.sections?.map((section: SectionItem, idx: number) => {
                        const numberStr = `٠${idx + 1}`.slice(-2).replace('1', '١').replace('2', '٢').replace('3', '٣').replace('4', '٤').replace('5', '٥').replace('6', '٦').replace('7', '٧').replace('8', '٨').replace('9', '٩').replace('0', '٠'); // basic arabic conversion
