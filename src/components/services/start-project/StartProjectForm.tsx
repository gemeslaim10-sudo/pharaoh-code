...
interface StartProjectFormProps {
  /** Company phone from settings/identity (already resolved on the server). */
  phone: string;
  address_ar: string;
  address_en: string;
}

export default function StartProjectForm({ phone, address_ar, address_en }: StartProjectFormProps) {
  const { language, direction } = useTranslation();
  const address = language === 'ar' ? (address_ar || address_en) : (address_en || address_ar);
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-5 p-6 bg-deepBlue/40 border border-white/5 rounded-3xl group hover:border-pharaohGold/30 transition-all">
                            ...
                            <div>
                                ...
                                {phone && (
                                  <a href={`tel:${phone.replace(/[^0-9+]/g, '')}`} className="text-gray-400 text-sm hover:text-pharaohGold transition-colors" dir="ltr">{phone}</a>
                                )}
                            </div>
                        </div>
