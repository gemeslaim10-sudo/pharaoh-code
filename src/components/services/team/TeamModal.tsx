...
export default function TeamModal() {
  ...
  return (
    <div id="team-profile-modal" className="fixed inset-0 z-[250] hidden bg-[#0A192F]/90 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6">
      <div className="w-full max-w-4xl bg-[#112240] border-t-2 border-r-2 border-[#C5A16F]/40 rounded-3xl md:rounded-tr-[4rem] md:rounded-bl-[4rem] shadow-[0_25px_60px_rgba(197,161,111,0.15)] relative overflow-hidden max-h-[90vh] sm:max-h-[85vh] flex flex-col text-right" dir="rtl">
        ...
        <div className="p-4 sm:p-8 md:p-12 overflow-y-auto custom-modal-scroll flex-1 relative">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-right gap-5 sm:gap-8 md:gap-10 border-b border-[#C5A16F]/10 pb-6 sm:pb-8 mb-6 sm:mb-8 relative z-10">
            <div className="w-32 h-40 sm:w-44 sm:h-56 rounded-2xl sm:rounded-tr-[3rem] sm:rounded-bl-[3rem] overflow-hidden p-1 border-2 border-[#C5A16F]/40 shadow-2xl shrink-0 bg-[#0A192F]">
              <img id="modal-member-img" className="w-full h-full object-cover rounded-xl sm:rounded-tr-[2.8rem] sm:rounded-bl-[2.8rem]" alt="Member" />
            </div>
            <div className="flex-1">
              <span id="modal-member-role-tag" className="inline-block bg-[#C5A16F]/10 text-[#C5A16F] text-[10px] sm:text-xs font-bold px-3.5 py-1 sm:py-1.5 rounded-lg tracking-wider uppercase border border-[#C5A16F]/20 shadow-sm" />
              <h2 id="modal-member-name" className="text-2xl sm:text-3xl md:text-4xl font-black text-white mt-3 sm:mt-4 mb-2 sm:mb-3 tracking-tight" />
              <p className="text-gray-300 text-xs sm:text-sm leading-relaxed max-w-2xl font-light" id="modal-member-desc" />
            </div>
          </div>
            ...
            <div className="flex flex-col justify-between gap-5 sm:gap-6">
              ...
              <div className="border-t border-[#C5A16F]/10 pt-4 sm:pt-5">
                ...
                <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
                  <a id="link-fb" href="#" target="_blank" className="flex-1 py-2.5 sm:py-3 px-4 rounded-xl bg-[#0A192F] text-gray-200 hover:text-[#C5A16F] hover:border-[#C5A16F]/80 border border-white/10 transition-all text-center text-xs font-bold tracking-wide shadow-md">
                    Facebook
                  </a>
                  <a id="link-insta" href="#" target="_blank" className="flex-1 py-2.5 sm:py-3 px-4 rounded-xl bg-[#0A192F] text-gray-200 hover:text-[#C5A16F] hover:border-[#C5A16F]/80 border border-white/10 transition-all text-center text-xs font-bold tracking-wide shadow-md">
                    Instagram
                  </a>
                </div>
              </div>
