...
export default function TeamProcess() {
  ...
  return (
    <section id="our-process" className="py-24 bg-[#050D1A] relative overflow-hidden" dir={direction}>
      {/* Background */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-10">
            {STEPS.map((step, index) => (
              <div key={index} className="flex flex-col items-center text-center group relative">
                ...
                <div className={`relative w-28 h-28 rounded-full bg-gradient-to-br ${step.color} border-2 ${step.borderColor} flex items-center justify-center mb-8 text-[#C5A16F] transition-all duration-500 group-hover:scale-110 shadow-2xl group-hover:shadow-[0_0_40px_rgba(197,161,111,0.25)] z-10 bg-[#0A192F]`}>
                  {step.icon}
                  {/* Ring pulse on hover */}
                  <div className="absolute inset-0 rounded-full border-2 border-[#C5A16F]/20 scale-150 opacity-0 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500" />
                </div>

                {/* Card below */}
                <div className={`relative w-full bg-gradient-to-b from-[#0F1E38] to-[#081222] rounded-3xl border border-white/6 ${step.borderColor} p-7 transition-all duration-500 overflow-hidden shadow-xl group-hover:-translate-y-2 group-hover:shadow-[0_20px_60px_rgba(0,0,0,0.4)]`}>
                  {/* Top glow */}
                  ...
                  <h3 className="text-xl font-black text-white mb-3 group-hover:text-[#C5A16F] transition-colors duration-300">
                    {t(step.titleKey)}
                  </h3>
