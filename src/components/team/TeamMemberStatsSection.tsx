  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:gap-4">
      {stats.map((stat: any, idx: number) => {
        ...
        // In English mode, format %97 as 97% for natural international formatting
        const displayValue = language === 'en' && rawValue.startsWith('%') 
          ? `${rawValue.slice(1)}%` 
          : rawValue;
        const displayLabel = getDynamicText(stat, 'label', language) 
